...
### [Paper](https://web.cecs.pdx.edu/~qiqi2/files/papers/vfips/ECCV2022_Qiqi_VideoQualityMetric_CameraReady.pdf) | [Data](https://github.com/hqqxyy/VFIPS#training) | [Video](https://web.cecs.pdx.edu/~qiqi2/files/papers/vfips/Video_Demo_Final.mp4) | [Poster](https://web.cecs.pdx.edu/~qiqi2/files/papers/vfips/poster.pdf) 
...
### Training 
1. Download the VFIPS dataset (Sorry, we realized that some source videos are deleted by their authors, we are cleaning them. Please contact with qiqi2 AT pdx DOT edu for it.). Set the path in the function `get_dataset_dir` in `utils.py`
```python
    if dataset == 'vfips':
